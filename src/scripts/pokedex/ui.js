...
export function renderPokemonCard(pokemon) {
  const image = pokemon.image
    ? pokemon.image
    : "https://placehold.co/200x200/f1f5f9/0f172a?text=No+image";
  const title = toTitleCase(pokemon.name);
  ...
  return `
    <article class="rounded-lg border border-slate-200 bg-white p-4">
      ...
      <img class="mx-auto mt-4 h-32 w-32 object-contain" src="${image}" alt="${title}" loading="lazy" />
      <div class="mt-4">
...
export function renderItemCard(item) {
  const image = item.image
    ? item.image
    : "https://placehold.co/120x120/e2e8f0/0f172a?text=Item";
